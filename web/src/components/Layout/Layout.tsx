...
import { 
  ...
  GitBranch, Code, Moon, Sun, Globe, Github
} from 'lucide-react';
...
export const Layout: React.FC = () => {
  ...
  return (
    <div className="flex h-screen bg-gray-50 dark:bg-gray-900 overflow-hidden">
      ...
      <motion.aside
        className={clsx(
          "fixed inset-y-0 left-0 z-40 w-64 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 shadow-sm md:relative transform transition-transform duration-300 ease-in-out flex flex-col",
      ...
      >
        <div className="p-6 border-b border-gray-100 dark:border-gray-700">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{t('sidebar.subtitle')}</p>
        </div>

        <div className="flex-1 overflow-y-auto py-4 px-3 space-y-6 scrollbar-thin scrollbar-thumb-gray-200 dark:scrollbar-thumb-gray-600">
          {categories.map((cat) => (
            groupedItems[cat] && (
              <div key={cat}>
                {cat !== 'Main' && (
        ...
        <div className="p-4 border-t border-gray-100 dark:border-gray-700">
          <div className="flex justify-center gap-4 mb-4">
              <button onClick={toggleTheme} className="p-2 text-gray-500 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full transition-colors" title={t('sidebar.tooltips.toggleTheme')}>
