    ...
    <div className="flex flex-col gap-6 p-6">
      <div className="flex flex-wrap gap-4 items-center bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 rounded-lg hover:bg-purple-200 dark:hover:bg-purple-800 transition-colors font-medium border border-purple-200 dark:border-purple-700"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Visualization Area */}
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 min-h-[400px] flex flex-col relative overflow-auto bg-grid-slate-100 dark:bg-grid-slate-900/50">
          
          {/* Method Area (Static) */}
          <div className="mb-8">
            <div className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-2">{t('labs.staticMembers.methodArea')}</div>
            <motion.div 
              className={`inline-flex items-center gap-4 bg-purple-50 dark:bg-purple-900/20 border-2 ${activeHighlight === 'static' ? 'border-purple-500 dark:border-purple-500 bg-purple-100 dark:bg-purple-900/40 scale-105' : 'border-purple-200 dark:border-purple-800'} rounded-xl p-4 transition-all duration-300`}
            >
              <div className="w-10 h-10 bg-purple-200 dark:bg-purple-800 rounded-lg flex items-center justify-center text-purple-700 dark:text-purple-300">
                <Box size={20} />
              </div>
              <div>
                <div className="text-xs text-purple-600 dark:text-purple-400 font-mono">{t('labs.staticMembers.staticVar')}</div>
                <div className="text-2xl font-bold text-purple-900 dark:text-purple-100">{globalCount}</div>
              </div>
            </motion.div>
          </div>

          {/* Heap Area (Instances) */}
          <div className="flex-1 bg-gray-50 dark:bg-gray-900/50 rounded-xl border border-dashed border-gray-300 dark:border-gray-700 p-4">
            <div className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-4">{t('labs.staticMembers.heapMemory')}</div>
            ...
            <div className="flex flex-wrap gap-4">
              <AnimatePresence>
                {instances.map((inst) => (
                  <motion.div
                    ...
                    className={`bg-white dark:bg-gray-800 rounded-xl shadow-sm border-2 ${activeHighlight === inst.id ? 'border-blue-500 dark:border-blue-500 ring-2 ring-blue-200 dark:ring-blue-900' : 'border-blue-100 dark:border-blue-900'} p-4 w-48 transition-all duration-300`}
                  >
                    <div className="flex justify-between items-start mb-3">
                      <div className="font-mono text-sm font-bold text-blue-800 dark:text-blue-300">c{inst.id}</div>
                      <div className="text-xs text-gray-400 dark:text-gray-500">{t('labs.staticMembers.reference')}</div>
                    </div>
                    
                    <div className="space-y-3">
                      {/* Instance Variable */}
                      <div className="bg-blue-50 dark:bg-blue-900/20 p-2 rounded-lg border border-blue-100 dark:border-blue-800">
                        <div className="text-xs text-blue-600 dark:text-blue-400 font-mono mb-1">{t('labs.staticMembers.instanceVar')}</div>
                        <div className="flex justify-between items-center">
                          <span className="text-lg font-bold text-blue-900 dark:text-blue-100">{inst.count}</span>
                          <button 
                      ...
                      <div className="border-t border-gray-100 dark:border-gray-700 pt-2">
                        <div className="text-[10px] text-gray-400 dark:text-gray-500 flex items-center gap-1">
                          ...
                          <span className="font-mono text-purple-600 dark:text-purple-400 font-bold">{globalCount}</span>
                        </div>
                      </div>
                    </div>
                  </motion.div>
              ...
              {instances.length === 0 && (
                <div className="w-full h-32 flex items-center justify-center text-gray-400 dark:text-gray-500 italic text-sm">
